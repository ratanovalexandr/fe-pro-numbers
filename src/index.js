...
export const getFixedValue = (number, accuracy) => {
    let num = Math.round (number * ( 10**accuracy))/ 10**accuracy;
    return num;
};
...
export const randomizer = (from, to) => {
            // from = Math.ceil(from);
        // to = Math.floor(to);
        // return Math.floor(Math.random() * (from - to)) + to; //Максимум не включается, минимум включается
      

        from = Math.ceil(from);
        to = Math.floor(to);
        return Math.floor(Math.random() * (from - to + 1)) + to; //Максимум и минимум включаются
}; 
